...
export interface UseColorModeOptions<T extends string = BasicColorMode> extends UseStorageOptions<T | BasicColorMode> {
   ...
   * @see https://ajiu9.cn/comuse/core/useColorMode/#advanced-usage
   */
  emitAuto?: boolean
 ...
 *
 * @see https://ajiu9.cn/comuse/core/useColorMode
 * @param options
 */
export function useColorMode<T extends string = BasicColorMode>(
  ...
  const {
    ...
    storage,
    storageKey = 'comuse-color-scheme',
  ...
  // Inline system preference detection (instead of using usePreferredDark)
  const preferredDark = shallowRef(false)
  ...
  const updateHTMLAttrs = getSSRHandler(
    'updateHTMLAttrs',
    (selector: string | MaybeElementRef, attribute: string, value: string) => {
