...
import * as metadata from 'comuse-metadata'
...
const SKILL_DIR = r('./skills/comuse-functions')

const SKILL_COPY_DIR = r('../../skills/comuse-functions')
const SKILL_REFERENCE_DIR = './references'
const SKILLS_TEMPLATE_PATH = r('./templates/comuse-functions-skills.md')
const COMUSE_ROOT = r('../..')
...
async function prepareFunctionReferences(outDir: string, referenceDir = SKILL_REFERENCE_DIR): Promise<Record<string, FunctionReference[]>> {
  ...
  for (const category of metadata.categoryNames) {
    ...
    for (const fn of functions) {
      ...
      const docPath = path.join(COMUSE_ROOT, `packages/${fn.package}/${fn.name}/index.md`)
